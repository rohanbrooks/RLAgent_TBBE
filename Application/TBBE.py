...
csv_file = "rl_learning_curve.csv"
...
class Session:
    ...
    def populateMarket(self):
        ...
        def initAgent(name, quantity, id):
            ...
            if name == 'RLBettingAgent': return RLBettingAgent(id, name, self.lengthOfRace, self.endOfInPlayBettingPeriod, 0, local_opinion, uncertainty, MIN_OP, MAX_OP, load_model_path="trained_dqn_model_20k.pth")
    ...
    def eventSession(self, simulationId):
        ...
        for id, agent in self.bettingAgents.items():
            if isinstance(agent, RLBettingAgent):
                metrics = agent.finalise_episode(winning_competitor=self.winningCompetitor)
                if metrics:
                    metrics["simulation"] = simulationId
...
class BBE(Session):
    ...
    def runSession(self, argFunc=None):
        ...
        while currentSimulation < NUM_OF_SIMS:
            ...
            if hasattr(self.session, "metrics_log"):
                all_metrics.extend(self.session.metrics_log)
            
            for agent_id, agent in self.session.bettingAgents.items():
                agent_statistics.append({
                    "simulation_id": currentSimulation,
                    "agent_id": agent_id,
                    "agent_type": agent.name,
                    "final_balance": agent.balance,
                    "num_transactions": len(agent.trades)
                })
            
            random.seed(currentSimulation)
            np.random.seed(currentSimulation)
            
            currentSimulation = currentSimulation + 1
            
        output_file = "agent_summary_statistics.csv"
       
        with open(output_file, mode='w', newline='') as file:
            writer = csv.DictWriter(file, fieldnames=agent_statistics[0].keys())
            writer.writeheader()
            writer.writerows(agent_statistics)    
        
        df = pd.DataFrame(all_metrics)
        df.to_csv("rl_learning_curve.csv", index=False)
        print(f"Wrote {len(df)} metrics rows to rl_learning_curve.csv")
